...
describe('FoundationStack', () => {
  ...
  describe('VPC', () => {
    ...
    it('should have correct CIDR block', () => {
      template.hasResourceProperties('AWS::EC2::VPC', {
        CidrBlock: testConfig.network.cidr,
      });
    });
    ...
    it('should have the correct name tag', () => {
      template.hasResourceProperties('AWS::EC2::VPC', {
        Tags: [
          {
            Key: 'Name',
            Value: `${testConfig.envName}-cdk-template-vpc`,
          },
        ],
      });
    });
  ...
  describe('Subnets', () => {
    it('should create public subnets', () => {
      // Public subnets = maxAzs * 1
      template.resourceCountIs('AWS::EC2::Subnet', testConfig.network.maxAzs * 3); // public + private + isolated
    });
    ...
    it('should create NAT Gateways', () => {
      // NAT Gateway count = natGateways config
  ...
  describe('Outputs', () => {
    ...
    it('should export VPC CIDR', () => {
      const outputs = template.findOutputs('*');
      expect(Object.keys(outputs)).toContain('VpcCidr');
    });
  });
